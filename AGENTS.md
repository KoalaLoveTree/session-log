...
In: `/` (text box, submit, recent list, edit, delete with confirm) and `/deleted` (restore), one `App.tsx` (no React Router), one table (`id`, `body`, `created_at`, `deleted_at`), `POST`/`GET` `/api/entries`, `GET /api/entries/deleted`, `PUT`/`DELETE /api/entries/{id}` (soft-delete), `POST /api/entries/{id}/restore`, `DELETE /api/entries/{id}/purge`, `docker compose up` → `http://localhost:3000`.
